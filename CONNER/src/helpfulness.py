...
def read_hyp_knowledge(path):
    '''读取生成的知识（由DPR或LLM生成）的前300条'''
    with open(path, 'r', encoding='utf-8') as r:
        lines = r.readlines()  # 读取所有行到列表中
    res = [line.strip() for line in lines[:300]]  # 只处理前300行
    assert len(res) == 300, f"Length of results is {len(res)} instead of 300"
    return res
...
def load_model(model_name):
    if 'flan' in model_name:
        ...
        path = '/share/home/202320143730/models/LLaMA-7B'
        tokenizer = LlamaTokenizer.from_pretrained(path, padding_side='left') # left-padding for decoder-only model
        tokenizer.pad_token, tokenizer.bos_id, tokenizer.eos_id = '<pad>', 1, 2
        model = LlamaForCausalLM.from_pretrained(path, torch_dtype=torch.float16)
    else:
...
def parse_args():
    ...
    parser.add_argument('--baseline_loss', type=float, default='')
...
def calculate_helpfulness(real_losses, baseline_loss_mean):
    """
    根据给定的真实损失列表和基线损失的平均值计算Helpfulness得分。
    
    :param real_losses: 使用真实知识生成答案的损失列表。
    :param baseline_loss_mean: 使用不相关知识（基线）生成答案的平均损失。
    :return: Helpfulness得分列表。
    """
    helpfulness_scores = [
        max(0, 1 - (real_loss / baseline_loss_mean)) for real_loss in real_losses
    ]
...
if __name__ == '__main__':
    ...
    for i in tqdm.tqdm(range(len(testset))):
        ...
        if args.debug:
            ...
            print (loss, ppl)
    helpfulness_scores = calculate_helpfulness(loss_list, args.baseline_loss)

    with open(f'/share/home/202320143730/models/med-proj/LLM_YL/CONNER/log/helpfulness/{args.exp_name}-f.txt', 'w') as f:
        ...
        f.write(f'Mean Helpfulness Score: {np.mean(helpfulness_scores)}\n')
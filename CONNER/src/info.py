...
tokenizer = AutoTokenizer.from_pretrained('/share/home/202320143730/models/gpt-neo-2.7B')
model = AutoModelForCausalLM.from_pretrained('/share/home/202320143730/models/gpt-neo-2.7B').half()
...
if __name__ == '__main__':
    ...
    assert len(hyps) == len(testset) == len(queries) == 300, (len(hyps), len(testset))

    info_list = calculate_info_per_example(hyps, queries, topics, args)
    assert len(info_list) == 300, len(info_list)

    print ('mean info = ', np.nanmean(info_list))
    
    with open('/share/home/202320143730/models/med-proj/LLM_YL/CONNER/log/info/' + args.task + '_info', 'w') as outfile:

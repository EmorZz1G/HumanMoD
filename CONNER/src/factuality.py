    ...
    if 'json' not in ref_path: # txt: for wow
        with open(ref_path, 'r') as infile:
            for line in infile:
                ...
                text = parts[2]
                truncated_text = truncate_text(text, max_length)
                doc_refs.append(truncated_text)
...
def args_parser():
    ...
    parser.add_argument('--IR_path', type=str, default=None, help='path to generations to evaluate') 
...
def main(args):
    ...
    IR_recalls = read_IR_docs(args.IR_path) 
    doc_refs = read_ref(args.ref_path) # txt file
    assert len(hyps) == len(doc_refs) == len(IR_recalls) == 300, (len(hyps), len(doc_refs), len(IR_recalls))
    ...
    res_path = './log/factuality' + f'_{args.outer_strategy}_factuality_results.txt'
    ...
    ana_path = './log/factuality' + f'_IR{args.retrieved_num}_{args.outer_strategy}_analysis.txt'
    with open(ana_path, 'w') as outfile:
        json.dump(all_analysis_list, outfile)
        outfile.write("\n")

    # save example NE score 
    ne_path = './log/factuality' + f'_IR{args.retrieved_num}_{args.outer_strategy}_example_NE.txt'
    with open(ne_path, 'w') as outfile:
        for ne in final_hallu_ner_score:
            outfile.write(str(ne) + '\n')

    # save example NLI score
    nli_path = "./log/factuality/" + f'_IR{args.retrieved_num}_{args.outer_strategy}_example_NLI_entail.txt'

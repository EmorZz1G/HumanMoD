...
def read_hyp(hyp_path, max_length=1024):
    hyps = []
    with open(hyp_path, 'r') as infile:
        for line in infile:
            truncated_line = line[:max_length].strip()
            hyps.append(truncated_line)
    return hyps

def read_testfile(testfile, max_length=1024):
    res = []
    with open(testfile, 'r', encoding='utf-8') as r:
        for i, line in enumerate(r):
            parts = line.strip().split('\t')
            assert len(parts) == 4, f"Expected 4 parts but got {len(parts)}: {parts}"
            truncated_parts = [part[:max_length] for part in parts]
            res.append(truncated_parts)
    return res
...
def single_instance_eval(hyp, query, answer, args):
    # multiple evidences
    hallu_ner_ratio = []
    ...
    if query.strip()[-1] != '?':
        query = query.strip() + '?'
    premise = query + '\t' + answer
    hypothesis = query + '\t' + hyp

    premise_hypothesis_pairs = [[premise, hypothesis]]
    nli_probs, labels = [], []
    bz_nli_probs, bz_labels = nli_metric_batch(premise_hypothesis_pairs)
...
def main(args):
    ...
    assert len(hyps) == len(testset) == 300, (len(hyps), len(testset))
    ...
    for i in tqdm(range(len(hyps))):
        hyp, example = hyps[i], testset[i]
        query, answer = example[1], example[3]

        res_obj = single_instance_eval(hyp, query, answer, args)
    ...
    res_path = '/share/home/202320143730/models/med-proj/LLM_YL/CONNER/log/validity/pub_factuality_results.txt'
    ...
    ana_path = '/share/home/202320143730/models/med-proj/LLM_YL/CONNER/log/validity/pub_analysis.txt'

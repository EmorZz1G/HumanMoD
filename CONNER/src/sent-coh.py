...
model = AutoModelForCausalLM.from_pretrained('/share/home/202320143730/models/gpt-neo-2.7B')
...
if __name__ == '__main__':
    ...
    assert len(sentences) == 300, len(sentences)
    ppls = calculate_ppls(sentences, device)
    assert len(ppls) == 300, len(ppls)

    inverse_ppls = [1 / p for p in ppls]
    coh_sent = np.nanmean(inverse_ppls)

    with open(args.hyp_path + './log/coherence/pub_avg_sent_ppl', 'w') as outfile:
